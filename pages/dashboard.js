          ...
          {view === 'batches' && (
            <div style={cardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                ...
                <button onClick={() => { setBatchForm(EMPTY_BATCH); setBatchError(''); setBatchPhotoFiles([]); setShowBatchForm(true); }} style={btnStyle}>
                  + New Batch
                </button>
              </div>
              {batches.length === 0 ? (
                  ...
                  {batches.map(batch => (
                    <button key={batch.id} onClick={() => { setActiveBatch(batch); loadBatchListings(batch.id); setView('batch-detail'); }} style={{
                      ...
                      {batch.photo_urls?.length > 0 && (
                        <span style={{ fontSize: 12, color: '#888' }}>📷 {batch.photo_urls.length}</span>
                      )}
            ...
            <>
              <div style={{ ...cardStyle, borderLeft: `4px solid ${DIFFICULTY.find(d => d.key === activeBatch.difficulty)?.color || GREEN}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  {activeBatch.photo_urls?.length > 0 && (
                    <span style={{ fontSize: 12, color: '#888' }}>📷 {activeBatch.photo_urls.length} photo{activeBatch.photo_urls.length !== 1 ? 's' : ''}</span>
                  )}
            ...
            <form onSubmit={handleCreateBatch}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea value={batchForm.comments} onChange={e => setBatchForm(f => ({ ...f, comments: e.target.value }))}
                  ...
                  placeholder="e.g. Titleist bag, mixed irons, some rust…" />
              </div>
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>Photos</label>
                <input ref={photoInputRef} type="file" multiple accept="image/*"
                  onChange={e => setBatchPhotoFiles(Array.from(e.target.files))}
                  style={{ fontSize: 13, width: '100%' }} />
                {batchPhotoFiles.length > 0 && (
                  <p style={{ fontSize: 12, color: '#555', marginTop: 4 }}>{batchPhotoFiles.length} photo{batchPhotoFiles.length !== 1 ? 's' : ''} selected</p>
                )}
              </div>
