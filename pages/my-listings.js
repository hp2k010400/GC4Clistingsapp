...
export default function MyListings({ profile, _debug }) {
  ...
  const loadHistory = useCallback(async () => {
    ...
    let query = supabase.from('listings').select('*, batches(difficulty, comments, description)').eq('user_id', profile.id).order('created_at', { ascending: false }).limit(500000);
  ...
  async function handleListingSubmit(e) {
    ...
    const { error: err } = await supabase.from('listings').insert({
      ...
      ...form, serial_id: form.serial_id.trim(), listing_value, product_type,
    });
      ...
      <div style={{ minHeight: '100vh', background: '#f4f6f4' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            ...
            <button onClick={() => setView('notes')} style={{ padding: '7px 18px', borderRadius: 7, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 13, background: view === 'notes' ? GREEN : '#e8eee8', color: view === 'notes' ? '#fff' : '#444', display: 'flex', alignItems: 'center', gap: 7 }}>
              Notes
              {unresolvedNotesCount > 0 && (
                <span style={{ background: view === 'notes' ? '#fff' : '#e67e22', color: view === 'notes' ? '#e67e22' : '#fff', borderRadius: 20, padding: '1px 7px', fontSize: 11, fontWeight: 800 }}>{unresolvedNotesCount}</span>
              )}
            </button>
            <button onClick={() => { setView('batches'); setActiveBatch(null); }} style={{ padding: '7px 18px', borderRadius: 7, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 13, background: view === 'batches' || view === 'batch-detail' ? GREEN : '#e8eee8', color: view === 'batches' || view === 'batch-detail' ? '#fff' : '#444' }}>+ Add Listings</button>
          </div>
